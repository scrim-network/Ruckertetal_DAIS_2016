...
png(file="Figures/Fig1_C_instPaleo.tif", family="Helvetica", width=6,
...
png(file="Figures/Fig2_C_instPaleo.tif", family="Helvetica", width=text_column_width,
...
plot(date[AD_1880], proj.mcmc.1961_1990[1,AD_1880], type="l", col="powderblue", lwd=2,
     xlab="Date [yr BP]", ylab="AIS Volume loss [SLE m]", 
     xlim=c(date[AD_1880], date[240010]), xaxt="n", ylim=c(-1,1)) #ylim=c(-2,3))
...
# lines(date, mcmc_995, lty=2, col="gray")
#2050
#lines(date,proj.mcmc.1961_1990[2500,] , col="gray", lwd=0.5)
lines(date,proj.mcmc.1961_1990[585,] , col="lightgray", lwd=0.5)
lines(date,proj.mcmc.1961_1990[1225,] , col="gray", lwd=0.5)
...
plot(date[240000], proj.mcmc.1961_1990[1,240000], type="l", col="powderblue", lwd=2,
     xlab="Date [yr BP]", ylab="AIS Volume loss [SLE m]", 
     xlim=c(-10,285), xaxt="n", ylim=c(-0.6,7)) #ylim=c(-2,7))
...
plot(date[240000], proj.mcmc.1961_1990[1,240000], type="l", col="powderblue", lwd=2,
     ...
     xlim=c(-15,106), ylim=c(-0.25,1.4), xaxt="n")#ylim=c(-0.1,1.35), xaxt="n")
...
abline(v=100, lty=2)
text(85, 0.4, paste(round(mcmc_mean[240100],2), "±", round(mcmc_std[240100],2), sep = ""), col="goldenrod2", cex=0.9)
...
#(8, 1.42)
legend(-12, 1.47, legend=c("This study neglecting cliff instability",
...
plot(density(DAIS_chains_burnin[,12]), main="", ylab = "", xlab=expression(sigma^{2}[P]), yaxt="n")
lines(density(sub_chain[,12]), col="red")
plot(density(DAIS_chains_burnin[,13]), main="", ylab = "", xlab=expression(sigma^{2}[I]), yaxt="n")
...
pairs.image <- function(x) {
    ...
    "P0", expression(kappa), "f0", "h0", "c","b0", "slope", expression(sigma^{2}[P]), expression(sigma^{2}[I])), panel=function(x,y) {
...
png(file="Scratch/Figures/Fig3_C_newinstPaleo.tif", family="Helvetica", width=text_column_width,
height=single_panel_height*2, units="in",pointsize=12, res=300)
par(mfrow=c(3,2), mgp=c(1.5,.5,0), mar=c(4, 4, 2, 1))
...
abline(h=1, lty=2)
text(1, 1+0.12, cex=0.9, "Instrumental period
Data Constraint
(relative to 1992)")#0.62
text(1, 1-0.1, cex=0.9, "Model Inversion
This study")
...
ylab="Probability Density",
xlim=c(-1,1.5),
ylim=c(-0.4,3.4), yaxt="n") #ylim=c(-7,53), yaxt="n") #2.4

lines(sf.2100.mcmc$pdf, col=this.study.color, lwd=1.5)

probs = c(0.05, 0.95)
add.hor.box(mcmc.prob_proj[,6], probs, width.size = 0.5, where.at = -.25, tick.length = 0.06, line.width = 1.5, color = this.study.color)

abline(h=1.9, lty=2)
text(1, 1.9+0.25, cex=0.9, "Expert Assessments")
text(1, 1.9-0.3, cex=0.9, "Model Inversion
This study")
...
legend("center", c("This study", "Median & 90% C.I. (Little et al. 2013)","Median & 90% C.I. (Church et al. 2013)",
"Median & 90% C.I. (Kopp et al. 2014)", "Mean & 90% C.I. (Pfeffer et al. 2008)","Median & 90% C.I.
(Bamber & Aspinall 2013)"), pch=15, bty="n", cex = 0.85,
col=c(this.study.color,expert_assessment))#, y.intersp=c(0.4,0.5,0.5,0.5, 0.5, 0.6))
