...
#------------------------------------------------
# -June 17, 2014 #Updates June 10 2015 ; Coded from R to Matlab March 7th 2016 ; Coded to Julia May 24th 2016
#------------------------------------------------

function iceflux(iceparam,forcings,standards,endinfo)
    ...
    Rad = zeros(enddate,1);            #Radius of ice sheet
    Vol = zeros(enddate,1);            #Ice volume
    SLE = zeros(enddate,1);            #Sea-level equivalent [m]
  ...
  #Setup the model
    ...
    #Apply whether or not the ice sheet has components of a marine ice sheet
            ...
            elseif R>rR && R<=rcon
                Btot = pi*P*R*R-pi*beta*(hr-bo+s*R)*(R*R-rR*rR)-(4*pi*beta*mu^0.5)/5*(R-rR)^2.5+(4*pi*beta*mu^0.5)/3*R*(R-rR)^1.5;
                F = 0;
                ISO = 0;
                fac = pi*(1+eps1)*(4/3*mu^0.5*R^1.5-s*R*R);
            elseif R<=rR && R>=rcon
